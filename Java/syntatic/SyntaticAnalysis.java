...
public class SyntaticAnalysis {
    ...
    private IfCommand procIf() {
        Expr expr; int line = lex.getLine();
        Command thenCmds; 
        IfCommand mainIfCommand = null;
        ...
        mainIfCommand = new IfCommand(line, expr, thenCmds);
        ArrayList<IfCommand> secondaryArray = new ArrayList<>(); int counter = 0;

        while(current.type == TokenType.ELSEIF || current.type == TokenType.ELSE){
            if(current.type == TokenType.ELSEIF){
                advance();
                Expr expr2 = procExpr();
                eat(TokenType.THEN);
                Command cmds2 = procCode();
                secondaryArray.add(new IfCommand(lex.getLine(), expr2, cmds2));
                
                if(counter > 0){
                    secondaryArray.get(counter - 1).setElseCommands(secondaryArray.get(counter));
                }
                ...
                Command cmds2 = procCode();
                if(counter >= 1)
                    secondaryArray.get(counter - 1).setElseCommands(cmds2);
                else
                    mainIfCommand.setElseCommands(cmds2);
            }
        }
        ...
        if(secondaryArray.size() > 0){
            mainIfCommand.setElseCommands(secondaryArray.get(0));
        }

        return mainIfCommand;
    }
